...
function renderMarkdown(md: string): string {
  ...
  // Helper: peek past blank lines to check if next non-blank line matches pattern
  ...
  while (i < lines.length) {
    ...
    if (/^[-*•] /.test(t)) {
      ...
      while (i < lines.length) {
        ...
        if (line === '') {
          // skip blank only if next non-blank is also a bullet
...
function renderDocument(md: string): string {
  // Split into sections by ## headings, give each a coloured card
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .tag-logo-svg rect { fill:#FC6C34; } .tag-logo-svg path { fill:white; }
</style>
</head><body>
